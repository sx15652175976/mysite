    ...
    <header class="header">
        ...
        <div class="navbar">  
            <div class="navbar-left-container">
                <a href="/">
                    <img class="navbar-brand-logo" src="../img/static/logo.png"></a>
            </div>
            <div class="navbar-right-container" style="display: flex;">
                <div class="navbar-menu-container">
                    ...
                    <!-- <a href="javascript:void(0)" class="navbar-link"  v-if="nickname">{{ nickname }}</a> -->
                    <a href="javascript:void(0)" class="navbar-link" @click="logout()">Logout</a>
                    <div class="navbar-cart-container">
                        <span class="navbar-cart-count" v-if="cartCount>0">{{ cartCount }}</span><!-- 当购物车中商品数量为0时,不能显示 -->
...
export default {
    ...
    data(){
        return{
            ...
            userPwd:'',//密码
            errorTip:'',//登录状态
            loginModalFlag:false,//登录框的显示隐藏
            //nickname:''//用户昵称--登陆后的  vuex第一步删除局部变量,用vuex中vuex中的state中的nickname替代
            

        }
    },
    mounted() { //生命周期
        this.checkLogin()//调用一个方法,检测用户有没有登录
        this.getCartCount() //调用购物车中商品函数
    },
    computed:{
        nickname(){ //替换掉data中的nickname,取代的是计算属性,现在还不行,因为$store.state.nickname还没有赋值
            return this.$store.state.nickname
            // console.log(this.$store)
        },
        cartCount(){//实时计算购物车中商品的数量
    ...
    methods:{
        ...
        getCartCount(){
            axios.get('/users/getCartCount').then((res)=>{
                ...
                // var res = res.data
                this.$store.commit('initCartCount',res.data.result) //把后台的数据传递到vuex中修改
            })
        ...
        checkLogin(){//判断登录状态
            axios.get('/users/checkLogin').then((res)=>{
                if(res.data.status=='0'){ //检测到登录用户,进行赋值
                    //this.nickname = res.data.result //cookie值  data中的nickname已经删除,所以不能使用
                    this.$store.commit('updateUserInfo',res.data.result) 
                    this.getCartCount() //如果登录状态,得知道购物车中商品数量
                }
            })
        },
        logout(){//退出登录
        ...
        login(){//登录
            ...
            axios.post("/users/login",{
                ...
                (response)=>{
                    let res = response.data//请求回来的值
                    // console.log(response.data.result.userName)
                    
                    if(res.status!="1"){//看后台  0-->成功  1-->失败
                        this.errorTip=false//请求成功让模态框隐藏
                        this.loginModalFlag=false
                        //this.nickname=response.data.result.userName  
                        this.$store.commit('updateUserInfo',res.result.userName)
                        this.getCartCount()
                    }else{
                        this.errorTip=true//不成功,显示
                    }
                }
            )
        }
