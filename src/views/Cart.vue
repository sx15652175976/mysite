<template>
  <div>
              ...
              <li v-for="(item,index) in cartList" :key='index'>
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    <!-- 商品选中状态  class  check -->
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{ 'check' :item.checked=='1' }" @click="enitCart('checked',item)">
                ...
                <div class="cart-tab-2">
                  <div class="item-price">{{ item.salePrice | c("￥") }}</div>
                </div>
                <div class="cart-tab-3">
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="enitCart('muit',item)" >-</a>
                        <span class="select-ipt">{{ item.productNum }}</span>
                        <a class="input-add" @click="enitCart('add',item)" >+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">{{ item.productNum * item.salePrice | c("￥") }}</div>
                </div>
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <!-- 删除时,传递的是整个购物车列表(item) -->
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <!-- 全选  -->
                <a href="javascipt:;" @click="toggleCheckAllFlag">
            ...
            <div class="cart-foot-r">
              <div class="item-total">
                Item total: <span class="total-price">{{ totalPrice | c("￥") }}</span>
              </div>
              <div class="btn-wrap">
                <!-- 当按钮class为btn--dis时,按钮不可用,没有勾选的商品的时候 -->
                <a class="btn btn--red" v-bind:class="{ 'btn--dis':totalPrice==0 }" @click="checkOut">Checkout</a>
                <!-- 当勾选的商品数量等于0时,空==0,所以不能结算 -->
              </div>
            </div>
          </div>
        </div>
    ...
    <Modal v-bind:mdShow="mdShow" v-on:close="closeModal"><!-- 自定义事件 -->
        <p slot="message">确定删除商品吗?</p>
        <div slot="btnGroup">
            <a class="btn btn-m" @click="delCart">确认</a>
            <a class="btn btn-m" @click="mdShow=false" >取消</a>
        </div>
    </Modal>
    <NavFooter />
  </div>
...
//import {currency} from '@/money/currency'//局部引入过滤器
    ...
    export default{
        data(){
            return{
                delItem:'',//点击删除后,此商品的参数
                mdShow:false,//模态框默认隐藏
                cartList:[],//购物车数据
                productId:'',//商品id
                //checkAllFlag:false//商品的状态 全选状态,自己的,关闭
            }
        },
        // filters:{ //过滤器
        //   c:currency
        ...
        computed:{
          ...
          checkedCount(){//计算是否全选,返回值是选中的数量
            var i = 0
            if(this.cartList.length==0){
              return
            }else{
              this.cartList.forEach((item)=>{
              if(item.checked=='1') i++ //记录购物车中有多少商品选中
            })
            }
          ...
          checkAllFlag(){
            return this.checkedCount==this.cartList.length//购物车的全部商品==你选中的商品状态的数量--->全选的状态变化(true/false)
          }
        },
        mounted() {
          this.init()
        ...
        methods: {
          checkOut(){
            if(this.checkedCount>0){//如果购物车中勾选的商品大于0
          ...
          //全选的状态
          toggleCheckAllFlag(){
            var checkAlls=!this.checkAllFlag//点击取反
            this.cartList.forEach(item=>{
              item.checked = checkAlls                              //效果是实现了,数据库还没变化(假数据)后台操作(数据库变化)
            })
            // console.log(this.checkAllFlag)
            axios.post('/users/editCheckAll',{checkAll:this.checkAllFlag}).then((res)=>{
              if(res.data.status=='0'){
                console.log('update selectAll suc!')
              }
            })
          },
          //购物车商品++--
          enitCart(flag,item){
            ...
            // console.log(item.productNum,item.productId)
            axios.post('/users/cartEnit',{
              productid:item.productId,
              productNum:item.productNum,
              checked:item.checked
            }).then((res)=>{ 
              console.log(res.data)
              let num = 0  //放在这里,是后台调用执行的
              //flag == 'add'?num = 1 : num = -1  //判断flag是+还是-(加=1,减=-1)
              if(flag == 'add'){
                num = 1
          ...
          delCartConfirm(item){//模态框显示
            this.delItem = item //整个购物车列表
            this.mdShow=true;
            this.productId=item.productId //给要删除的商品id
          },
          closeModal(){//模态框隐藏
            this.mdShow=false
          },
          delCart(){ //删除购物车
            //点击确认后,执行此方法
            axios.post("/users/cartDel",{productId:this.productId}).then((res)=>{
              //链式写法
              console.log(res.data)
              if(res.data.status=='0'){
                this.mdShow=false,
                this.init()
                this.$store.commit('updatecartCount',-this.delItem.productNum) //-1不对
              }
            })
          ...
          init(){
            ...
            this.$store.commit('updatePage','cart')//商品列表,给vuex中的state.page赋值为空''
