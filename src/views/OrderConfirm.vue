        ...
        <div class="item-list-wrap confirm-item-list-wrap">
          <div class="cart-item order-item">
            ...
            <ul class="cart-item-list">
              ...
              <li v-for="(item,index) in cartList" :key="index" v-show="item.checked=='1'">
                ...
                <div class="cart-tab-4">
                  <div class="item-price-total">{{ item.salePrice * item.productNum | c("￥") }}</div>
                </div>
              </li>
            </ul>
          </div>
...
import axios from 'axios'
  export default{
      ...
      methods: {
        payMent(){
          //获取从地址栏传过来的值(address路由中next按钮传递过来的值)
          var addressId = this.$route.query.addressId
          console.log(addressId)
          axios.post('/users/payMent',{ addressId:addressId,orderTotal:this.OrderTotal }).then((res)=>{
           ...
           axios.get('/users/cartList').then((res)=>{
            ...
            this.cartList.forEach(element =>{
              if(element.checked=="1"){
                this.Subtotal+=element.salePrice*element.productNum
