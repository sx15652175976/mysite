<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in AddressListFilter" :key="index" v-bind:class="{'check':checkIndex==index}" @click="checkIndex = index;selectAddressId=item.addressId">
                    <!-- 通过数组的下标和循环的索引对应,绑定class为check     当点击某个地址,改变index为当前地址的index -->
                    <dl>
                      <dt>{{ item.userName }}</dt>
              ...
              <div class="shipping-addr-more">
                <!-- 添加class为open,图标改变 -->
                <a class="addr-more-btn up-down-btn" href="javascript:;" @click="expand" v-bind:class="{ 'open':limit==AddressList.length }">
                  more
                  <i class="i-up-down">
      ...
      <Modal :md-show="isShow" v-on:close="closeModal"><!-- 自定义事件 -->
        <p slot="message">确定删除此地址吗?</p>
        <div slot="btnGroup">
            <a class="btn btn-m" @click="addressDel()">确认</a>
            <a class="btn btn-m" @click="isShow=false">取消</a>
        </div>
    </Modal>
    <Modal :md-show="isAdd" v-on:close="closeModal" class="addAddress"><!-- 自定义事件 -->
        <p slot="message">
        <label for="">添加您的新地址!</label>
        <input type="text" placeholder="请输入您的姓名" v-model="name">
        <input type="text" placeholder="请输入您的电话" v-model="tel">
        <input type="text" placeholder="请输入您的地址" v-model="address">
        <input type="text" placeholder="请输入您的邮编" v-model="postCode">
        </p>
        <div slot="btnGroup">
            <a class="btn btn-m" @click="addAddress">确认</a>
...
<style>
.addAddress input{ margin-top: 10px;display: block;margin-left: 15px;height: 35px;width: 400px;border-radius: 5px; }
</style>
<script>
  ...
  export default{
      data(){
          return{
            ...
            limit:3,//默认显示3条
            AddressList:[],//地址列表
            checkIndex:0,//默认第一个选中
            isShow:false,//默认隐藏模态框
            addressId:''
          }
      },
      mounted() {
      ...
      computed: {
        AddressListFilter(){
          return this.AddressList.slice(0,this.limit) //防止损坏原数据
        }
      },
      methods: {
        //添加的模态框
        ...
        //确定添加按钮
        addAddress(){
          axios.post('/users/addAddress',{userName:this.name,streetName:this.address,postCode:this.postCode,tel:this.tel,isDefault:false,addressId:new Date().getTime()+parseInt(Math.random()*100)}).then((res)=>{
        ...
        closeModal(){
          this.isShow=false
          this.isAdd=false//关闭添加地址的模态框
        },
        //删除指定地址
        addressDel(){
          axios.post('/users/AddressDel',{addressId:this.addressId}).then((res)=>{
            if(res.data.status==0){
                this.isShow=false
                this.init()
              }else{
        ...
        //默认显示条数
        expand(){
          if(this.limit==3){//判断this.limit为默认显示3条数据时
